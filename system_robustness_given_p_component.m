function [ robustness_index ] = system_robustness_given_p_component( p_component,beta_system,input )
%Find robustness index given p_component and beta_system
%   input
%       p : design parameter
%Initialize random varaibles
robustness_index=zeros(1,input.N_sim);
    ...
    parfor i_sim=1:input.N_sim
        ...
        if  or(input.sys_type(i_sim)==0, input.num_el(i_sim)==1)
            ...
            % Calculation of Pfcomp
            %                           beta_comp = component_beta_given_p_component( p_component );
            %                           Pf_comp=normcdf(-beta_comp);
